...
add_action('add_meta_boxes', function () {
    add_meta_box(
        'deheftruckchauffeur_split_hero_meta',
        'Split Hero instellingen',
        'deheftruckchauffeur_split_hero_meta_box',
        'page',
        'normal',
        'high'
    );
...
function deheftruckchauffeur_split_hero_meta_box(WP_Post $post): void {
    if (get_page_template_slug($post->ID) !== 'page-split-hero.php') {
        echo '<p style="color:#999;margin:0;">Alleen beschikbaar bij het paginasjabloon <strong>deheftruckchauffeur.nl Split Hero</strong>.</p>';
        return;
    }

    wp_nonce_field('deheftruckchauffeur_split_hero_save', 'deheftruckchauffeur_split_hero_nonce', false);
...
add_action('save_post', function (int $post_id): void {
    ...
    if (!isset($_POST['deheftruckchauffeur_split_hero_nonce'])) {
        return;
    }

    if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['deheftruckchauffeur_split_hero_nonce'])), 'deheftruckchauffeur_split_hero_save')) {
        return;
    }
...
function deheftruckchauffeur_split_hero_highlight(string $text, string $terms_raw): string {
