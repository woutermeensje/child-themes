...
/* ============================================================
   Custom Post Type: omj_vacature
   Slaat vacature-formulier inzendingen op in de database.
============================================================ */

add_action('init', function () {
    register_post_type('omj_vacature', [
        'labels' => [
...
add_filter('manage_omj_vacature_posts_columns', function ($columns) {
    return [
        'cb'          => $columns['cb'],
        'title'       => 'Vacaturetitel',
        'sj_pakket'   => 'Pakket',
...
add_action('manage_omj_vacature_posts_custom_column', function ($column, $post_id) {
    switch ($column) {
        case 'sj_pakket':
...
add_filter('manage_edit-omj_vacature_sortable_columns', function ($columns) {
...
add_action('add_meta_boxes', function () {
    add_meta_box('omj_vacature_details', 'Inzendingsdetails', 'omj_vacature_details_meta_box', 'omj_vacature', 'normal', 'high');
    add_meta_box('omj_vacature_omschrijving', 'Vacature omschrijving', 'omj_vacature_omschrijving_meta_box', 'omj_vacature', 'normal', 'default');
    add_meta_box('omj_vacature_logo', 'Bedrijfslogo', 'omj_vacature_logo_meta_box', 'omj_vacature', 'side', 'default');
});

function omj_vacature_details_meta_box($post) {
    $fields = [
    ...
    .sj-meta-table td a { color: #7C5CFA; }
    .sj-meta-badge { display: inline-block; padding: 3px 10px; border-radius: 999px; background: #e0d9ff; color: #7C5CFA; font-size: 12px; font-weight: 700; }
    </style>
...
function omj_vacature_omschrijving_meta_box($post) {
...
function omj_vacature_logo_meta_box($post) {
    ...
    if ($logo_id) {
        ...
        if ($logo_url) {
            ...
            echo '<p style="margin:8px 0 0;"><a href="' . esc_url(get_edit_post_link($logo_id)) . '" target="_blank" style="font-size:12px;color:#7C5CFA;">Bekijk in mediabibliotheek</a></p>';
        }
    } else {
...
add_action('admin_notices', function () {
    ...
    if (!$screen || $screen->post_type !== 'omj_vacature') return;

    $count   = wp_count_posts('omj_vacature');
