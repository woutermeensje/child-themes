...
function fn_ac_request(string $method, string $endpoint, array $body = []): ?array {
    ...
    if (!$base_url || !$api_key) {
        error_log('[Fondsen AC] Geen API-credentials geconfigureerd.');
        return null;
    }
    ...
    if (is_wp_error($response)) {
        error_log('[Fondsen AC] API-fout: ' . $response->get_error_message());
        return null;
    }
    ...
    if ($code >= 400) {
        error_log('[Fondsen AC] HTTP ' . $code . ' op ' . $endpoint . ': ' . wp_remote_retrieve_body($response));
        return null;
    }
...
function fn_ac_send_newsletter_campaign(array $vacatures): bool {
    ...
    $week_nr = date('W');
    $jaar    = date('Y');
    $subject = "De nieuwste vacatures van de afgelopen twee weken — Fondsen.org";
    $html    = fn_build_newsletter_email_ac($vacatures);

    $msg = fn_ac_request('POST', 'messages', [
        'message' => [
            'fromname'       => 'Fondsen.org',
            'fromemail'      => 'nieuwsbrief@fondsen.org',
            'reply2'         => 'nieuwsbrief@fondsen.org',
            'subject'        => $subject,
    ...
    if (empty($msg['message']['id'])) {
        error_log('[Fondsen AC Newsletter] Message aanmaken mislukt.');
        return false;
    }
    ...
    $campaign = fn_ac_request('POST', 'campaigns', [
        'campaign' => [
            ...
            'status'     => 1,
            'public'     => 0,
            'name'       => "Fondsen.org Nieuwsbrief week {$week_nr}, {$jaar}",
    ...
    if (empty($campaign['campaign']['id'])) {
        error_log('[Fondsen AC Newsletter] Campaign aanmaken mislukt.');
        return false;
    }

    error_log('[Fondsen AC Newsletter] Campaign ' . $campaign['campaign']['id'] . ' aangemaakt voor week ' . $week_nr . '/' . $jaar);
    return true;
}
...
<head>
  ...
  <title>Vacaturenieuwsbrief — Fondsen.org</title>
</head>
      ...
      <table width=\"600\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:600px;width:100%;background:#ffffff;border-radius:8px;overflow:hidden;border:1px solid #E0E0E0;\">
        ...
        <tr>
          <td style=\"background:#0884CC;padding:24px 32px;\">
            <span style=\"font-family:Arial,sans-serif;font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;\">Fondsen.org</span>
        ...
        <tr>
          <td style=\"padding:28px 32px 8px;\">
            ...
            <p style=\"margin:0;font-size:15px;color:#333333;line-height:1.65;font-family:Arial,sans-serif;\">Dit zijn de nieuwste vacatures van de afgelopen twee weken op Fondsen.org. Ontdek jouw volgende stap in de non-profit sector.</p>
          </td>
        </tr>
