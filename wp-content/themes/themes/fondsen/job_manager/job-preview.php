...
<form method="post" id="job_preview" action="<?php echo esc_url($form->get_action()); ?>">
    ...
    <div class="job_listing_preview single_job_listing">
        ...
        <?php get_job_manager_template_part('content-job_listing', 'job_listing'); ?>

        <input type="hidden" name="job_id" value="<?php echo esc_attr($form->get_job_id()); ?>" />
        <input type="hidden" name="step" value="<?php echo esc_attr($form->get_step()); ?>" />
        <input type="hidden" name="job_manager_form" value="<?php echo esc_attr($form->get_form_name()); ?>" />
        <hr />
        <input type="submit" name="continue" id="job_preview_submit_button" class="btn btn-color-139258 job-manager-button-submit-listing" value="<?php echo esc_attr(apply_filters('submit_job_step_preview_submit_text', __('Vacature insturen', 'wp-job-manager'))); ?>" />
        <input type="submit" name="edit_job" class="btn btn-accent job-manager-button-edit-listing" value="<?php esc_attr_e('Edit listing', 'wp-job-manager'); ?>" />
    </div>
    <?php
    /**
