...
add_filter( 'manage_mh_information_request_posts_columns', 'mh_information_request_columns' );
...
add_action( 'manage_mh_information_request_posts_custom_column', 'mh_information_request_column_content', 10, 2 );
...
function mh_store_information_request( array $data ): int {
	...
	$post_id = wp_insert_post(
		array(
			'post_type'   => 'mh_information_request',
			'post_status' => 'publish',
...
function mh_information_request_shortcode( array $atts = array() ): string {
	...
	if (
		...
		&& wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mh_information_nonce'] ) ), 'mh_information_request' )
	) {
	...
	<div class="mh-info-form">
		...
		<form method="post" novalidate id="<?php echo esc_attr( $form_id ); ?>">
			<div class="mh-info-form__body">
				<?php wp_nonce_field( 'mh_information_request', 'mh_information_nonce' ); ?>
