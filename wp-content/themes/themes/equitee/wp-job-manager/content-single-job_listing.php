...
<div class="custom-top-section">
    <div class="top-section-text">
        <p>
            Blijf op de hoogte van de nieuwste IT vacatures!
        </p>
     <div>
        <a href="/vacature-alert/" class="top-section-link">Vacature Alert instellen</a>
      </div>
    </div>
</div>
    ...
    <?php if ( $recent_jobs->have_posts() ) : ?>
      <section class="sj-recent sj-card">
        <div class="sj-recent-head">
          ...
          <p class="sj-recent-sub">Bekijk de nieuwste IT vacatures in ons netwerk.</p>
        </div>
...
.custom-top-section{ width:100%; max-width:900px; margin:24px auto; color:#333; border:1px solid #DEDEDE; box-shadow:0px 10px 40px -5px rgba(0,0,0,0.15); border-radius:5px; background:#fff; }
.top-section-text{ padding:20px 24px; display:flex; align-items:center; gap:16px; }
.top-section-text > div{ flex:0 0 auto; }
.top-section-text p{ margin:0; flex:1 1 auto; font-family:Poppins,system-ui,sans-serif; font-size:18px; color:#333; font-weight:600; }
.top-section-link{ display:inline-flex; align-items:center; justify-content:center; padding:10px 14px; border-radius:8px; background:rgba(124,92,250,0.12); color:var(--color-primary) !important; border:1px solid var(--color-primary); text-decoration:none !important; font-family:Poppins,system-ui,sans-serif; font-weight:600; font-size:14px; transition:transform .15s ease,box-shadow .15s ease; }
.top-section-link:hover{ transform:translateY(-1px); box-shadow:0 10px 40px -5px rgba(0,0,0,0.15); }
...
.sj-chip--link:hover{ border-color:rgba(124,92,250,.35); }

.sj-header{ padding-bottom:14px; border-bottom:1px solid var(--sj-border); margin-bottom:16px; margin-top:24px; }
.sj-title{ margin:0; font-family:Inter,system-ui,sans-serif; font-size:26px; line-height:1.15; font-weight:800; color:var(--sj-ink); max-width:100%; white-space:normal !important; overflow-wrap:anywhere; word-break:break-word; hyphens:auto; }
...
.sj-btn{ display:inline-flex; align-items:center; justify-content:center; padding:10px 14px; border-radius:10px; background:rgba(124,92,250,0.12); color:var(--color-primary) !important; text-decoration:none !important; border:1px solid var(--color-primary); font-family:Poppins,system-ui,sans-serif; font-weight:700; font-size:14px; box-shadow:0 10px 40px -5px rgba(0,0,0,0.15); transition:transform .15s ease,filter .15s ease; }
...
.sj-recent-item:hover{ transform:translateY(-3px); border-color:rgba(124,92,250,.35); box-shadow:0 18px 44px rgba(16,24,40,.14); }
...
.sj-recent-company{ font-family:Poppins,system-ui,sans-serif; font-size:13px; font-weight:700; color:var(--sj-blue); }
...
@media (max-width:768px){
  *, *::before, *::after{ box-sizing:border-box; }
  .top-section-text{ flex-direction:column; align-items:stretch; padding:16px; }
  .top-section-link{ width:100%; }
  .custom-top-section{ margin:16px auto; }
