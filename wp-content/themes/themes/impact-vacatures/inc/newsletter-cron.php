...
add_filter('cron_schedules', function (array $schedules): array {
    if (!isset($schedules['fn_biweekly'])) {
        $schedules['fn_biweekly'] = [
            ...
            'display'  => 'Eén keer per twee weken (Impact Vacatures)',
        ];
    }
    return $schedules;
});
...
function fn_send_biweekly_newsletter(): void {
    ...
    if (empty($vacatures)) {
        error_log('[Impact Vacatures Newsletter] Geen nieuwe vacatures de afgelopen 14 dagen, verzending overgeslagen.');
        return;
    }
...
add_action('init', function () {
    ...
    $message = $updated
        ? 'Je bent succesvol afgemeld voor de nieuwsbrief van Impact Vacatures.'
