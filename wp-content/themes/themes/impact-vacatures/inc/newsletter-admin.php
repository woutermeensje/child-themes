...
add_action('admin_menu', function () {
    add_management_page(
        'Impact Vacatures — Job Alerts & Nieuwsbrief',
        'Impact Vacatures — Mailings',
        'manage_options',
        'fn-newsletter',
...
function fn_newsletter_admin_handle_actions(): ?string {
    ...
    if ($action === 'test_job_alert') {
        ...
        $sent  = wp_mail(
            $email,
            '[TEST] Nieuwe vacatures in jouw vakgebied — Impact Vacatures',
            fn_build_job_alert_email($voornaam, $vacatures, $token),
            ['Content-Type: text/html; charset=UTF-8', 'From: Impact Vacatures <nieuwsbrief@impactvacatures.nl>']
        );
    ...
    if ($action === 'test_newsletter') {
        ...
        $sent = wp_mail(
            $email,
            '[TEST] De nieuwste vacatures van de afgelopen twee weken — Impact Vacatures',
            $body,
            $headers
        );
  ...
  <tr><td align=\"center\">
    <table width=\"600\" cellpadding=\"0\" cellspacing=\"0\" style=\"max-width:600px;width:100%;background:#ffffff;border-radius:8px;overflow:hidden;border:1px solid #E0E0E0;\">
      <tr><td style=\"background:#0884CC;padding:24px 32px;\">
        <span style=\"font-family:Arial,sans-serif;font-size:20px;font-weight:700;color:#ffffff;\">Impact Vacatures</span>
      ...
      <tr><td style=\"padding:28px 32px 8px;\">
        ...
        <p style=\"margin:0;font-size:15px;color:#333333;line-height:1.65;font-family:Arial,sans-serif;\">Dit zijn de nieuwste vacatures van de afgelopen twee weken op Impact Vacatures.</p>
      </td></tr>
...
function fn_newsletter_admin_page(): void {
    ...
    $ac_list_id     = defined('IMPACT_VACATURES_AC_NEWSLETTER_LIST_ID') ? (int) IMPACT_VACATURES_AC_NEWSLETTER_LIST_ID : 0;
    $sector_terms   = get_terms(['taxonomy' => 'job_sector', 'hide_empty' => false, 'orderby' => 'name']) ?: [];
    ?>
    <div class="wrap">
        <h1>Impact Vacatures — Job Alerts &amp; Nieuwsbrief</h1>
        <?php echo $message; ?>
