<?php
/**
 * Single blogpost — Studentinhuren NL child theme
 *
...
<?php
    ...
    $auteur    = get_post_meta( get_the_ID(), '_si_artikel_auteur', true );
    $thumb     = get_the_post_thumbnail(
        get_the_ID(),
        'full',
        [
            'class' => 'si-single__hero-img-media',
...
<main id="content" class="site-main si-single-page">
<div class="si-single__wrap">

    <!-- ── Artikel ─────────────────────────────────────── -->
    <article <?php post_class( 'si-single__article' ); ?>>

        <?php if ( $cat_lbl ) : ?>
            <a href="<?php echo $cat_url; ?>" class="si-single__cat"<?php echo $cat_style ? ' style="' . esc_attr( $cat_style ) . '"' : ''; ?>><?php echo $cat_lbl; ?></a>
        <?php endif; ?>

        <h1 class="si-single__title"><?php the_title(); ?></h1>

        <div class="si-single__meta">
            <?php if ( $auteur ) : ?>
                <span class="si-single__auteur">
            ...
            <span class="si-single__readtime">
        ...
        <?php if ( $thumb ) : ?>
            <div class="si-single__hero-img">
                <?php echo $thumb; ?>
            </div>
        <?php endif; ?>

        <div class="si-single__content">
            <?php the_content(); ?>
            <?php wp_link_pages(); ?>
        </div>

        <?php if ( has_tag() ) : ?>
            <div class="si-single__tags">
                <?php the_tags( '', '' ); ?>
            </div>
        <?php endif; ?>

        <nav class="si-single__nav" aria-label="Artikelnavigatie">
            <div class="si-single__nav-prev">
                <?php previous_post_link( '%link', '&larr; %title' ); ?>
            </div>
            <div class="si-single__nav-next">
    ...
    <aside class="si-single__sidebar">

        <div class="si-single__recent">
            <h3 class="si-single__recent-title">Recente artikelen</h3>
            <?php
            $recent = new WP_Query( [
                'post_type'      => 'post',
                'posts_per_page' => 6,
            ...
            if ( $recent->have_posts() ) :
                while ( $recent->have_posts() ) : $recent->the_post();
                    $r_thumb  = get_the_post_thumbnail(
                        get_the_ID(),
                        'thumbnail',
                        [
                            'class' => 'si-recent__img',
                            'sizes' => '72px',
                        ]
                    );
                    ...
                    $r_cat_style = ( $r_cat_term && function_exists( 'si_get_blog_category_style' ) ) ? si_get_blog_category_style( $r_cat_term ) : '';
            ?>
            <a href="<?php the_permalink(); ?>" class="si-recent__item">
                <div class="si-recent__img-wrap">
                    <?php if ( $r_thumb ) : ?>
                        <?php echo $r_thumb; ?>
                    <?php else : ?>
                        <div class="si-recent__img-placeholder">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#2f5f80" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="m3 9 4-4 4 4 4-4 4 4"/><circle cx="8.5" cy="14.5" r="1.5"/><path d="m21 15-5-5-5 5"/></svg>
                        </div>
                    <?php endif; ?>
                </div>
                <div class="si-recent__body">
                    <?php if ( $r_cat ) : ?>
                        <span class="si-recent__cat"<?php echo $r_cat_style ? ' style="' . esc_attr( $r_cat_style ) . '"' : ''; ?>><?php echo $r_cat; ?></span>
                    <?php endif; ?>
                    <h4 class="si-recent__title"><?php the_title(); ?></h4>
                    <span class="si-recent__date"><?php echo esc_html( $r_date ); ?></span>
                </div>
            </a>
            <?php
                endwhile;
        ...
        <div class="si-sidebar-newsletter" style="margin-top:24px;">
