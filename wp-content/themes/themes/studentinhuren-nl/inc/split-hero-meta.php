...
add_action('add_meta_boxes', function () {
    add_meta_box(
        'studentinhuren_split_hero_meta',
        'Split Hero instellingen',
        'studentinhuren_split_hero_meta_box',
        'page',
        'normal',
        'high'
    );
...
function studentinhuren_split_hero_meta_box(WP_Post $post): void {
    if (get_page_template_slug($post->ID) !== 'page-split-hero.php') {
        echo '<p style="color:#999;margin:0;">Alleen beschikbaar bij het paginasjabloon <strong>Studentinhuren.nl Split Hero</strong>.</p>';
        return;
    }

    wp_nonce_field('studentinhuren_split_hero_save', 'studentinhuren_split_hero_nonce', false);
...
add_action('save_post', function (int $post_id): void {
    ...
    if (!isset($_POST['studentinhuren_split_hero_nonce'])) {
        return;
    }

    if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['studentinhuren_split_hero_nonce'])), 'studentinhuren_split_hero_save')) {
        return;
    }
...
function studentinhuren_split_hero_highlight(string $text, string $terms_raw): string {
