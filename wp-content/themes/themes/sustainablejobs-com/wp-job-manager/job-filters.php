...
if ( ! function_exists( 'sj_get_jobs_filter_page_featured_image_url' ) ) {
  function sj_get_jobs_filter_page_featured_image_url( $size = 'full' ) {
    $page_id = is_singular() ? get_queried_object_id() : 0;

    if ( ! $page_id && ! empty( $GLOBALS['post']->ID ) ) {
      $page_id = (int) $GLOBALS['post']->ID;
    }
    ...
    return get_the_post_thumbnail_url( $page_id, $size ) ?: '';
  }
}

$hero_image = sj_get_jobs_filter_page_featured_image_url();
$hero_count_args = [
...
<div class="sj-job-hero"<?php if ( $hero_image ) : ?> style="background-image: url('<?php echo esc_url( $hero_image ); ?>');"<?php endif; ?>>
    <div class="sj-job-hero__inner">
        <span class="sj-job-hero__eyebrow">Sustainablejobs.com</span>
        <div class="sj-job-hero__title-wrap">
            <h1 class="sj-job-hero__title">Job board for sustainability, energy transition, ecology and roles with positive impact.</h1>
        </div>
        <p class="sj-job-hero__subtitle">Browse all <a href="#job_listings" class="sj-job-hero__jobs-link"><span class="sj-hero-job-count sj-job-hero__accent"><?php echo esc_html( number_format_i18n( $hero_job_count ) ); ?></span> jobs</a> or sign up for our <a href="/newsletter/" class="sj-job-hero__link">job newsletter</a>.</p>
    </div>
</div>
  ...
  <div class="filter-row">
    ...
    <div class="job_type">
      <select name="filter_job_types[]" id="filter_job_types"
              ...
              data-placeholder="Employment type"
              multiple>
    ...
    <div class="job_organisatie_type">
      <select name="filter_organisatie_type[]" id="filter_organisatie_type"
              ...
              data-placeholder="Organization type"
              multiple>
  ...
  const renderActiveFilters = () => {
    ...
    document.querySelectorAll("select.js-custom-select").forEach((select) => {
      ...
      selectedOptions.forEach((opt) => {
        ...
        chip.setAttribute("title", "Remove filter");
  ...
  const buildSelect = (select) => {
    ...
    btn.innerHTML = `
      ...
      <span class="sj-actions">
        <button type="button" class="sj-clear" aria-label="Clear selection" title="Clear selection">×</button>
...
jQuery(function($) {
  const updateHeroJobCount = (count) => {
    ...
    $(".sj-hero-job-count").text(new Intl.NumberFormat(document.documentElement.lang || "en-US").format(parsed));
  };
...
.sj-job-hero::before {
  ...
  position: absolute;
  inset: 0;
  background: rgba(37, 79, 110, 0.70);
  z-index: 1;
  pointer-events: none;
}
...
.sj-job-hero__eyebrow {
  ...
  margin: 0 0 14px;
  padding-bottom: 6px;
  border-bottom: 3px solid #9BCB86;
...
.sj-job-hero__jobs-link {
  color: #ffffff !important;
  text-decoration: none !important;
}

.sj-job-hero__jobs-link:hover {
  color: rgba(255, 255, 255, 0.8) !important;
}

.sj-job-hero__jobs-link .sj-job-hero__accent {
  color: #9BCB86 !important;
}
