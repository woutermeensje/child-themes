...
if (!function_exists('sj_get_job_favorites_nav_link')) {
    function sj_get_job_favorites_nav_link(string $class = ''): string {
        ...
        return sprintf(
            '<a href="%1$s" class="%2$s" data-sj-favorites-nav-link aria-label="My saved jobs"><span class="sj-favorites-nav__icon">%3$s</span><span class="sj-favorites-nav__label">My saved jobs</span><span class="sj-favorites-nav__count" data-sj-favorites-count hidden>0</span></a>',
            esc_url(home_url('/mijn-jobs/')),
            esc_attr($classes),
            sj_get_favorites_heart_svg()
        );
    }
...
add_action('init', function () {
    add_rewrite_tag('%sj_mijn_jobs%', '1');
    add_rewrite_rule('^mijn-jobs/?$', 'index.php?sj_mijn_jobs=1', 'top');
...
add_filter('template_include', function ($template) {
    if ((int) get_query_var('sj_mijn_jobs') !== 1) {
        return $template;
    }

    $favorites_template = get_stylesheet_directory() . '/page-mijn-jobs.php';
    return file_exists($favorites_template) ? $favorites_template : $template;
});

add_filter('document_title_parts', function ($title) {
    if ((int) get_query_var('sj_mijn_jobs') === 1) {
        $title['title'] = 'My saved jobs';
    }
