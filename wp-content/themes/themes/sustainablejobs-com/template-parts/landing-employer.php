...
$intro           = has_excerpt($page_id) ? get_the_excerpt($page_id) : 'Sustainablejobs.com is the dedicated job board and content platform for professionals and organizations in sustainability, energy transition, ecology, biodiversity, nature conservation, non-profits, government, municipalities and NGOs.';
$primary_label   = get_post_meta($page_id, 'landing_primary_button_text', true)   ?: 'Post a Job';
$primary_url     = get_post_meta($page_id, 'landing_primary_button_url', true)    ?: home_url('/post-a-job/');
$secondary_label = get_post_meta($page_id, 'landing_secondary_button_text', true) ?: 'View Packages';
$secondary_url   = get_post_meta($page_id, 'landing_secondary_button_url', true)  ?: home_url('/pricing/');
$contact_phone   = get_post_meta($page_id, 'landing_phone', true) ?: '085 239 2040';
$contact_email   = get_post_meta($page_id, 'landing_email', true) ?: 'team@sustainablejobs.com';
?>
