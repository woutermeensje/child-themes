<?php
/**
 * Blog overview page — Sustainablejobs COM child theme
 *
 * Layout:
 *  1. Two full-width hero blocks connected to the navigation
 *  2. Lower section: list of articles and newsletter sidebar
 */
if ( ! defined( 'ABSPATH' ) ) {
    exit;
}
...
// With an active search: no hero blocks, all results as list rows.
if ( $zoekterm !== '' ) {
    $top_posts = [];
    $list_rows = $posts_all;
} else {
...
<main id="content" class="site-main sj-blog-page">
    ...
    <?php else : ?>
        ...
        // --------------------------------------------------------------------
        // 1. Two full-width hero blocks connected to the header.
        // --------------------------------------------------------------------
        if ( ! empty( $top_posts ) ) :
        ?>
        ...
        <?php
        // --------------------------------------------------------------------
        // 2. Lower section with list and sidebar wrapper.
        // --------------------------------------------------------------------
        ?>
        <div class="sj-blog__lower">
            ...
            <div class="sj-blog__main">
                ...
                <form class="sj-blog__search" role="search" method="get" action="<?php echo esc_url( $blog_url ); ?>">
                    <div class="sj-blog__search-wrap">
                        ...
                        >
                        <?php if ( $zoekterm ) : ?>
                        <a href="<?php echo esc_url( $blog_url ); ?>" class="sj-blog__search-clear" aria-label="Clear search" title="Clear search">
                        ...
                        <?php endif; ?>
                        <button type="submit" class="sj-blog__search-btn">Search</button>
                    </div>
                </form>

                <?php if ( $zoekterm !== '' && empty( $list_rows ) ) : ?>
                    <p class="sj-blog__empty">No articles found for <strong><?php echo esc_html( $zoekterm ); ?></strong>.</p>
                <?php elseif ( ! empty( $list_rows ) ) : ?>
                <?php
                    $n = count( $list_rows );
                    $list_title = $zoekterm !== ''
                        ? $n . ' ' . ( $n === 1 ? 'result' : 'results' ) . " for '" . esc_html( $zoekterm ) . "'"
                        : 'All articles';
                ?>
            ...
            <aside class="sj-blog__sidebar">
                ...
                <div class="sj-sidebar-links">
                    ...
                    <a href="https://platform.sustainablejobs.com/register" class="sj-sidebar-link" target="_blank" rel="noopener noreferrer">
                        <span class="sj-sidebar-link__icon" aria-hidden="true">
