(function () {
    ...
    function updateButtons(jobs) {
        ...
        document.querySelectorAll('[data-sj-favorite-button]').forEach((button) => {
            ...
            const label = active ? 'Verwijder uit mijn vacatures' : 'Bewaar vacature';
            ...
            if (visibleLabel) {
                visibleLabel.textContent = active ? 'Opgeslagen' : 'Bewaar vacature';
            }
        });
    }
    ...
    function updateCounts(jobs) {
        ...
        document.querySelectorAll('[data-sj-favorites-nav-link]').forEach((link) => {
            ...
            link.setAttribute('aria-label', count > 0 ? `Mijn vacatures (${count})` : 'Mijn vacatures');
        });
    }
