...
// =========================================================
// ITP_Nav_Walker – dropdown-indicator voor navigatie
// =========================================================
if ( ! class_exists('ITP_Nav_Walker') ) :
class ITP_Nav_Walker extends Walker_Nav_Menu {
...
// =========================================================
// =========================================================
// Shortcode: [itp_header]
// =========================================================
add_shortcode('itp_header', function() {
    ob_start();
...
// =========================================================
if ( ! function_exists('itp_get_req_value') ) {
    function itp_get_req_value($key) {
...
add_filter('job_manager_get_listings_shortcode_args', function($atts){
    global $itp_job_shortcode_atts;
    $itp_job_shortcode_atts = $atts;
...
add_filter('get_job_listings_query_args', function ($query_args, $args) {
    ...
    if ( ! empty($itp_job_shortcode_atts) && empty($_POST['form_data']) ) {
        foreach ($custom_taxonomies as $filter_key => $taxonomy) {
            $key = str_replace('filter_', '', $filter_key);
            if ( ! empty($itp_job_shortcode_atts[$key]) ) {
                $terms = array_map('sanitize_title', explode(',', sanitize_text_field($itp_job_shortcode_atts[$key])));
                $query_args['tax_query'][] = [
                    'taxonomy' => $taxonomy,
                    'field'    => 'slug',
                    'terms'    => $terms,
