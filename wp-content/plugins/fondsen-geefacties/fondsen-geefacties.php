 ...
 * Description: Geefacties (donatieverzoeken) met listing + single template.
 * Version: 0.1.3
 */

if (!defined('ABSPATH')) exit;

define('FGA_VERSION', '0.1.3');
...
final class FGA_Plugin {
  ...
  public function __construct() {
    ...
    // Alleen thumbnails support aanzetten (geen extra metabox forceren!)
    ...
    // Details metabox (benodigd kapitaal + stichting)
    add_action('add_meta_boxes', [$this, 'add_details_metabox']);
    add_action('save_post_' . self::CPT, [$this, 'save_details_metabox'], 10, 2);

    // Includes (als deze bestaan in jouw plugin)
    $q = FGA_PATH . 'includes/class-fga-query.php';
    $s = FGA_PATH . 'includes/class-fga-shortcode.php';

    if (file_exists($q)) require_once $q;
    if (file_exists($s)) require_once $s;

    if (class_exists('FGA_Shortcode')) {
      new FGA_Shortcode();
    }
  }

  public function enable_thumbnails_support() {
    // Zorgt dat je in Gutenberg rechts 1x "Uitgelichte afbeelding" hebt.
  ...
  public function save_contact_metabox($post_id, $post) {
    ...
    if (!isset($_POST['fga_contact_nonce']) || !wp_verify_nonce($_POST['fga_contact_nonce'], 'fga_contact_save')) return;
  ...
  public function save_details_metabox($post_id, $post) {
    ...
    if (!isset($_POST['fga_details_nonce']) || !wp_verify_nonce($_POST['fga_details_nonce'], 'fga_details_save')) return;
    if (!current_user_can('edit_post', $post_id)) return;
